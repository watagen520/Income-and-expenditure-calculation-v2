...
def _get_or_create_spark() -> SparkSession:
    """Spark Sessionを取得。Spark Connect切断時も再生成を試みる。"""
    session = SparkSession.getActiveSession()
    if session is not None:
        return session

    # Databricks Connect環境向けフォールバック
    try:
        from databricks.connect import DatabricksSession  # type: ignore

        return DatabricksSession.builder.getOrCreate()  # type: ignore[return-value]
    except Exception:
        return SparkSession.builder.getOrCreate()


def _run_statement_with_retry(spark: SparkSession, stmt: str) -> SparkSession:
    """セッション期限切れ時に1回だけ再接続してSQLを再実行する。"""
    try:
        spark.sql(stmt)
        return spark
    except Exception as e:
        msg = str(e)
        if "NO_ACTIVE_SESSION" in msg or "No active Spark session found" in msg:
            refreshed = _get_or_create_spark()
            refreshed.sql(stmt)
            return refreshed
        raise
...
def _run_sql_file(spark: SparkSession, path: Path) -> SparkSession:
    if path.suffix.lower() != ".sql":
        raise RuntimeError(
            f"SQL実行対象が .sql ではありません: {path}. "
            "manual_batch_medallion.py は Python として %run で実行してください。"
        )
    if not path.exists():
    ...
    for stmt in _split_sql_statements(text):
        current = _run_statement_with_retry(current, stmt)
    return current
...
def main() -> None:
    ...
    print(f"Build SQL: {build_sql}")
    print(f"Gate SQL: {gate_sql}")
    print(f"Cutover SQL: {cutover_sql}")
    print("[1/3] Build: canonical + Gold _v2 を構築します")
    spark = _run_sql_file(spark, build_sql)

    print("[2/3] Gate: 受け入れ判定テーブルを更新します")
    spark = _run_sql_file(spark, gate_sql)

    print("[3/3] Cutover: v2専用運用へ切替えます")
    _prepare_cutover_targets(spark)
    spark = _run_sql_file(spark, cutover_sql)
