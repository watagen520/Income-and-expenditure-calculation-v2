...
checks AS (
  ...
  SELECT 'E1_unclassified_ratio_le_20pct',
         CASE WHEN MAX(unclassified_share) <= 0.20 THEN 'PASS' ELSE 'FAIL' END,
         CONCAT('max_unclassified_share=', CAST(MAX(unclassified_share) AS STRING))
  FROM (
    SELECT
      year_month,
      CASE WHEN SUM(spend_amount)=0 THEN 0.0
           ELSE SUM(CASE WHEN category_lv1 IN ('その他','未分類') THEN spend_amount ELSE 0 END)/SUM(spend_amount)
      END AS unclassified_share
  ...
  SELECT 'F2_owner_coverage_check',
         CASE WHEN MIN(owner_scope_count) >= 1 THEN 'PASS' ELSE 'FAIL' END,
         CONCAT('min_owner_scope_count=', CAST(MIN(owner_scope_count) AS STRING))
  FROM (
    SELECT year_month, COUNT(DISTINCT owner_scope) AS owner_scope_count
    FROM watanabe.finance_gold.monthly_spend_by_owner_v2
    GROUP BY year_month
  ) oc
